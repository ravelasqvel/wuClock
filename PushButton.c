 ...
 * \returns 
 */
void PBCatchEventFSM(push_button_t *PB){
    ...
    if(gpioValue){
        ...
        gpio_acknowledge_irq(PB->BITS.gpioNum);
    }
}
 ...
 * \returns 
 */
void PBDebounceFSM1(push_button_t *PB){
 ...
 * \returns 
 */
void PBDebounceFSM2(push_button_t *PB){
    bool gpioValue = gpio_get(PB->BITS.gpioNum);
    if(!gpioValue){
 ...
 * \returns 
 */
void PBDebounceFSM3(push_button_t *PB){
    if(tb_check(&(PB->pbTBDebouncer))){
        uint32_t gpioEvent = gpio_get_irq_event_mask(PB->BITS.gpioNum);
        gpio_acknowledge_irq(PB->BITS.gpioNum);
        if(!gpioEvent){
 ...
 * \returns 
 */
void PBCatchEventNFSM(push_button_t *PB){
